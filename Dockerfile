...
RUN \
  DIR=/tmp/webp && \
        git clone -b 1.0.2 --single-branch --depth 1 https://github.com/webmproject/libwebp.git ${DIR} ; \
        cd ${DIR} ; \
        libtoolize --force --copy -i ; \
        ./autogen.sh || (libtoolize --copy && ./autogen.sh) ; \
        ./configure --prefix="${PREFIX}" --enable-shared=no ; \
        make -j 4 ; \
        make install ; \
        sudo cp "${PREFIX}/bin/dwebp" /usr/local/ ; \
        make distclean ; \
        rm -rf ${DIR}
